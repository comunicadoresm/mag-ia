...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Build agent tags map (agent_id -> array of tag_ids)
  ...
  // Filter agents by search query and selected tag
  const filteredAgents = agents.filter((agent) => {
    ...
    const matchesTag =
      activeTag === null || (agentTags[agent.id] && agentTags[agent.id].includes(activeTag));

    return matchesSearch && matchesTag;
  });

  // Build a set of tag IDs that have at least one active agent
  const tagsWithAgents = new Set<string>();
  agents.forEach((agent) => {
    if (agentTags[agent.id]) {
      agentTags[agent.id].forEach((tagId) => tagsWithAgents.add(tagId));
    }
  });

  // Filter tags to only show those with agents
  const visibleTags = tags.filter((tag) => tagsWithAgents.has(tag.id));

  // Get the title based on active tag
  const getTitle = () => {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredAgents.map((agent) => (
                <button
                  ...
                  className="card-cm-interactive p-5 text-left group"
                >
                  <div className="flex items-start gap-4 mb-4">
                    <div className="icon-circle text-2xl shrink-0">
                      {agent.icon_emoji || '🤖'}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {agentTags[agent.id]?.map((tagId) => {
                      ...
                      return tag ? (
                        <span
                          key={tagId}
                          className="text-xs px-3 py-1 rounded-full border border-primary/40 text-primary font-medium"
                        >
                          {tag.name}
                        </span>
                      ) : null;
