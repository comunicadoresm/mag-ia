...
interface FormatQuizFlowProps {
  onComplete: () => void;
  onSkip: () => void;
}

// Same scoring logic as original FormatQuizSetup — only copy/UI updated
const EXPERIENCE_OPTIONS = [
  { value: 'never', label: 'Nunca gravei nada', score: 0 },
...
const TIME_SCRIPT = [
  ...
  { value: '15_to_30', label: '15 a 30 min (gosta de construir junto)', score: 3 },
];

const TIME_RECORDING = [
  { value: 'up_to_10', label: 'Até 10 min (grava direto, sem repetir)', score: 1 },
  { value: '10_to_30', label: '10 a 30 min (2-3 takes)', score: 2 },
  { value: '30_to_60', label: '30 min a 1h (setup + múltiplos takes)', score: 3 },
...
const TIME_EDITING = [
  ...
  { value: '30_plus', label: '30 min+ (edição elaborada)', score: 3 },
];
...
const EQUIPMENT_CATEGORIES = {
  ...
  audio: {
    ...
    items: [
      ...
      { value: 'desk_mic', label: 'Microfone de mesa (USB)', score: 3 },
  ...
  extras: {
    label: '✨ EXTRAS',
    items: [
      { value: 'teleprompter', label: 'Teleprompter', score: 1 },
      { value: 'home_studio', label: 'Cenário/fundo dedicado', score: 3 },
      { value: 'chroma_key', label: 'Tela verde / chroma key', score: 3 },
      { value: 'crew', label: 'Alguém que filma pra mim', score: 5 },
    ],
  },
};
...
export function FormatQuizFlow({ onComplete, onSkip }: FormatQuizFlowProps) {
  const { user } = useAuth();
  const [step, setStep] = useState<'intro' | number | 'result'>('intro');
  const [answers, setAnswers] = useState<any>({
    experience: '', time_script: '', time_recording: '', time_editing: '',
    equipment: [] as string[], recording_apps: [] as string[], editing_apps: [] as string[],
    editing_level: '', desired_frequency: '',
  });
  ...
  const calculateResult = () => {
    ...
    if (answers.experience === 'never') format = 'low_fi';
    else if (recordingScore <= 1 && editingScore === 0) format = 'low_fi';
    else if (totalScore <= 8) format = 'low_fi';
    else if (totalScore <= 18) format = 'mid_fi';
    else format = 'hi_fi';
    ...
    const plan = days.map((day, i) => {
      ...
      return {
        day, format: dayFormat, suggestion: sugs[i % sugs.length],
  ...
  const handleFinish = async () => {
    ...
    setSaving(true);
    try {
      const { error } = await supabase.from('user_format_profile').upsert({
        user_id: user.id,
      ...
      <div className="space-y-6">
        <div className="text-center space-y-3">
          ...
          <h2 className="text-xl font-bold text-foreground">
            Qual formato sustentável é o SEU?
          </h2>
          <p className="text-sm text-muted-foreground leading-relaxed max-w-sm mx-auto">
            Vou te ajudar a descobrir o formato que você consegue manter <strong className="text-foreground">sem sofrer</strong>.
          </p>
        ...
        <div className="space-y-2 pt-2">
          <Button onClick={() => setStep(0)} className="w-full h-12 rounded-xl text-base font-semibold">Começar</Button>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex gap-1 mb-2">
          {[0, 1, 2, 3, 4].map(i => (
            <div key={i} className={`h-1.5 flex-1 rounded-full ${i <= step ? 'bg-primary' : 'bg-muted/30'}`} />
          ))}
        </div>

        {step === 0 && (
          <div className="space-y-2">
            <p className="text-sm font-medium text-foreground">Quando você se imagina gravando um vídeo, qual frase mais te representa?</p>
            {EXPERIENCE_OPTIONS.map(o => (
              <OptionButton key={o.value} selected={answers.experience === o.value} onClick={() => setAnswers((p: any) => ({ ...p, experience: o.value }))}>{o.label}</OptionButton>
            ))}
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-3">
            <p className="text-sm font-medium text-foreground">Quanto tempo por dia você consegue dedicar a conteúdo?</p>
            <div>
              <p className="text-xs font-semibold text-foreground mb-1.5">ROTEIRO:</p>
              {TIME_SCRIPT.map(o => (<div key={o.value} className="mb-1.5"><OptionButton selected={answers.time_script === o.value} onClick={() => setAnswers((p: any) => ({ ...p, time_script: o.value }))}>{o.label}</OptionButton></div>))}
            </div>
            <div>
              <p className="text-xs font-semibold text-foreground mb-1.5">GRAVAÇÃO:</p>
              {TIME_RECORDING.map(o => (<div key={o.value} className="mb-1.5"><OptionButton selected={answers.time_recording === o.value} onClick={() => setAnswers((p: any) => ({ ...p, time_recording: o.value }))}>{o.label}</OptionButton></div>))}
            </div>
            <div>
              <p className="text-xs font-semibold text-foreground mb-1.5">EDIÇÃO:</p>
              {TIME_EDITING.map(o => (<div key={o.value} className="mb-1.5"><OptionButton selected={answers.time_editing === o.value} onClick={() => setAnswers((p: any) => ({ ...p, time_editing: o.value }))}>{o.label}</OptionButton></div>))}
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-3">
            <p className="text-sm font-medium text-foreground">Me conta: o que você tem em mãos pra gravar?</p>
            {Object.entries(EQUIPMENT_CATEGORIES).map(([catKey, cat]) => (
              <div key={catKey}>
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {cat.items.map(item => (
                    <CheckboxButton key={item.value} selected={answers.equipment.includes(item.value)} onClick={() => toggleArray('equipment', item.value)}>{item.label}</CheckboxButton>
                  ))}
                </div>
              </div>
            ))}
        ...
        {step === 3 && (
          <div className="space-y-3">
            <p className="text-sm font-medium text-foreground">Quais apps de gravação e edição você usa?</p>
            <div>
              <p className="text-xs font-semibold text-foreground mb-1.5">GRAVAÇÃO:</p>
              <div className="grid grid-cols-2 gap-1.5">
                {RECORDING_APPS.map(a => (<CheckboxButton key={a.value} selected={answers.recording_apps.includes(a.value)} onClick={() => toggleArray('recording_apps', a.value)}>{a.label}</CheckboxButton>))}
              </div>
            </div>
            <div>
              <p className="text-xs font-semibold text-foreground mb-1.5">EDIÇÃO:</p>
              <div className="grid grid-cols-2 gap-1.5">
                {EDITING_APPS.map(a => (<CheckboxButton key={a.value} selected={answers.editing_apps.includes(a.value)} onClick={() => toggleArray('editing_apps', a.value)}>{a.label}</CheckboxButton>))}
              </div>
            </div>
            <div>
              <p className="text-xs font-semibold text-foreground mb-1.5">SEU NÍVEL DE EDIÇÃO:</p>
              {EDITING_LEVELS.map(o => (<div key={o.value} className="mb-1.5"><OptionButton selected={answers.editing_level === o.value} onClick={() => setAnswers((p: any) => ({ ...p, editing_level: o.value }))}>{o.label}</OptionButton></div>))}
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-2">
            <p className="text-sm font-medium text-foreground">Quantos vídeos por semana você REALMENTE consegue manter sem surtar?</p>
            {FREQUENCY_OPTIONS.map(o => (
              <OptionButton key={o.value} selected={answers.desired_frequency === o.value} onClick={() => setAnswers((p: any) => ({ ...p, desired_frequency: o.value }))}>{o.label}</OptionButton>
            ))}
          </div>
        )}
